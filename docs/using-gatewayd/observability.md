...
Plugins have their own log level settings, which can be configured in the [plugins configuration](plugins-configuration/plugins-configuration) file: `gatewayd_plugins.yaml`.
...
Plugins can choose to expose metrics to GatewayD. The metrics merger can be configured in the [plugins configuration](plugins-configuration/plugins-configuration) and the metrics merger can be configured in the [general plugin configuration](plugins-configuration/general-configurations).
...
Plugins can also generate traces, but they are not collected by GatewayD. The plugins can be configured to send traces to the supported backends individually in their [configuration](plugins-configuration/plugins-configuration) parameters.
